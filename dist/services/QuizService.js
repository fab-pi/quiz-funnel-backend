...
class QuizService {
    ...
    async getQuizContent(quizId) {
        ...
        try {
            // Get quiz info
            const quizResult = await client.query('SELECT quiz_id, quiz_name, product_page_url, is_active FROM quizzes WHERE quiz_id = $1', [parseInt(quizId)]);
     ...
     */
    async getDropRateAnalytics(quizId) {
        const client = await this.pool.connect();
        try {
     ...
     */
    async getUTMPerformanceAnalytics(quizId) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`
        SELECT 
