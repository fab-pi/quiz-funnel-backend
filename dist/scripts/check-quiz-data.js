...
const checkQuizData = async () => {
    try {
        ...
        // Check quiz data
        const quizResult = await db_1.default.query('SELECT quiz_id, quiz_name, product_page_url, is_active, brand_logo_url, color_primary, color_secondary, color_text_default, color_text_hover FROM quizzes WHERE quiz_id = 9');
        ...
        // Check all quizzes
        const allQuizzesResult = await db_1.default.query('SELECT quiz_id, quiz_name, color_primary, color_secondary FROM quizzes ORDER BY quiz_id');
        console.log('\n📊 All quizzes:');
...
//# sourceMappingURL=check-quiz-data.js.map