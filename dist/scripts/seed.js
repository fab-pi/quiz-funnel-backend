...
const seedData = async () => {
    try {
        ...
        const migrationCheck = await db_1.default.query('SELECT COUNT(*) FROM migrations');
        ...
        const quizResult = await db_1.default.query(`INSERT INTO quizzes (quiz_name, product_page_url, creation_date, is_active, 
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING quiz_id`, [
        ...
        for (const question of questions) {
            const result = await db_1.default.query(`INSERT INTO questions (quiz_id, sequence_order, question_text, interaction_type, 
          instructions_text, loader_text) 
         VALUES ($1, $2, $3, $4, $5, $6) RETURNING question_id`, [quizId, question.sequence_order, question.question_text, question.interaction_type,
        ...
        for (const option of answerOptions) {
            await db_1.default.query('INSERT INTO answer_options (question_id, option_text, associated_value) VALUES ($1, $2, $3)', [option.question_id, option.option_text, option.associated_value]);
        ...
        await db_1.default.query('UPDATE quizzes SET quiz_start_url = $1 WHERE quiz_id = $2', [quizStartUrl, quizId]);
