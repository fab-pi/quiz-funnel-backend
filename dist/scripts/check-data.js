...
const checkData = async () => {
    try {
        ...
        // Check quizzes
        const quizzes = await db_1.default.query('SELECT quiz_id, quiz_name, creation_date FROM quizzes ORDER BY quiz_id');
        ...
        // Check questions
        const questions = await db_1.default.query('SELECT question_id, quiz_id, question_text FROM questions ORDER BY quiz_id, sequence_order');
        console.log(`\n❓ Questions (${questions.rows.length}):`);
        questions.rows.forEach(q => {
            console.log(`   ID: ${q.question_id} | Quiz: ${q.quiz_id} | Text: ${q.question_text.substring(0, 50)}...`);
        });
        // Check user sessions
        const sessions = await db_1.default.query('SELECT session_id, quiz_id, start_timestamp FROM user_sessions ORDER BY session_id');
        ...
        // Check user answers
        const answers = await db_1.default.query('SELECT answer_id, session_id, question_id FROM user_answers ORDER BY answer_id');
